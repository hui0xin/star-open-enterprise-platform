package com.star.project.generator.simple;
...
import com.star.project.generator.standard.rest.EnvYamlGenerateUtil;
import com.star.project.generator.standard.rest.RestProjectGenerateUtil;
import com.star.project.generator.utils.FilePathUtils;
import com.star.project.generator.utils.FreeMarkerTemplateUtils;
...
public class SimpleProjectGenerateUtil {
    ...
    public static void generateSimpleProject(String filePath, String projectName, String projectDesc, Boolean isMultiEnv,
                                             Boolean isDockerProject) throws Exception {
        ...
        if (isDockerProject) {
            if (isMultiEnv) {
                //生成DockerFile
                generateSimpleDockerEnvPom(filePath, projectName, projectDesc);
                System.out.println("    1, ----生成 pom.xml 文件成功");
            } else {
                //生成DockerFile
                generateSimpleDockerPom(filePath, projectName, projectDesc);
                System.out.println("    1, ----生成 pom.xml 文件成功");
            }
        } else {
            if (isMultiEnv) {
                //生成DockerFile
                generateSimpleEnvPom(filePath, projectName, projectDesc);
                System.out.println("    1, ----生成 pom.xml 文件成功");
            } else {
                //生成简单pom.xml
        ...
        if (isDockerProject) {
            //生成DockerFile
            RestProjectGenerateUtil.generateDockerFile(filePath, projectName, projectDesc);
            System.out.println("    16, ----生成 DockerFile 文件成功");
        }
    }
     ...
     * 生成简单项目pom.xml
     */
    public static void generateSimpleEnvPom(String filePath, String projectName, String projectDesc) throws Exception {
        File prjectFilePom = new File(filePath + "/pom.xml");
        Template template = FreeMarkerTemplateUtils.getTemplate("/simple/pom-base-env.ftl");
    ...
    public static void generateSimplePom(String filePath, String projectName, String projectDesc) throws Exception {
        File prjectFilePom = new File(filePath + "/pom.xml");
        Template template = FreeMarkerTemplateUtils.getTemplate("/simple/pom-base.ftl");
        FileOutputStream fos = new FileOutputStream(prjectFilePom);
        Writer out = new BufferedWriter(new OutputStreamWriter(fos, "utf-8"), 10240);
        Map<String, Object> dataMap = new HashMap<String, Object>();
        String className = FilePathUtils.getClassName(projectName);
     ...
     * 生成简单项目pom.xml
     */
    public static void generateSimpleDockerPom(String filePath, String projectName, String projectDesc) throws Exception {
        File prjectFilePom = new File(filePath + "/pom.xml");
        Template template = FreeMarkerTemplateUtils.getTemplate("/simple/pom-docker.ftl");
        FileOutputStream fos = new FileOutputStream(prjectFilePom);
        Writer out = new BufferedWriter(new OutputStreamWriter(fos, "utf-8"), 10240);
        Map<String, Object> dataMap = new HashMap<String, Object>();
        String className = FilePathUtils.getClassName(projectName);
     ...
     * 生成简单项目pom.xml
     */
    public static void generateSimpleDockerEnvPom(String filePath, String projectName, String projectDesc) throws Exception {
        File prjectFilePom = new File(filePath + "/pom.xml");
        Template template = FreeMarkerTemplateUtils.getTemplate("/simple/pom-docker-env.ftl");
        FileOutputStream fos = new FileOutputStream(prjectFilePom);
        Writer out = new BufferedWriter(new OutputStreamWriter(fos, "utf-8"), 10240);
        Map<String, Object> dataMap = new HashMap<String, Object>();
        String className = FilePathUtils.getClassName(projectName);
