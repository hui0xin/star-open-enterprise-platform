...
@Update({
    "<script>" +
        ...
        " UPDATE" +
        " ylt_listen_rank" +
        " SET rank=${r'#{item.rank}'} WHERE sn = ${r'#{item.sn}'} " +
        "</foreach>" +
    "</script>"
})
...
"where geo_hash like CONCAT(${r'#{geoHash,jdbcType=BIGINT}'},'%')"
"<if test='content != null'> and content like CONCAT('%',${r'#{content,jdbcType=VARCHAR}'},'%')</if> "

"and create_time between ${r'#{startTime}'} and ${r'#{endTime}'} order by create_time asc"
...
import com.star.commons.support.utils.DateUtil;
## 2 使用 OkHttpClientUtil
import com.star.commons.support.utils.OkHttpClientUtil;
...
//前面的拷贝到后面
BeanUtils.copyProperties(tConfig, tConfigDataVo);
```
